...
function renderContourPanel() {
  ...
  $('contourTgl').checked = contourState.on;
...
function renderContourLegend() {
  ...
  const wanted = (!tgl || tgl.checked) && contourState.on && contourModel.ready;
...
function initContourPanel() {
  ...
  on('contourTgl', 'change', e => { setContourEnabled(e.target.checked); renderContourPanel(); });
